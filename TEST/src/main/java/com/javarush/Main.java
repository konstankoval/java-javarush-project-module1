...
public class Main {
    static void main() {
                ...
                final Map<Integer, Character> TEXT_TO_CEASAR;
                final Map<Character, Integer> CEASAR_TO_TEXT;
        ...
        System.out.println("\n\nВы ввели текст: " + text);
        System.out.println("Вы ввели ключ: " + code);

        // Перевод в верхний регистр
        String upperText = text.toUpperCase();

        // Перевели в массив char
        char[] charArray = upperText.toCharArray();
        List<Integer> intArray = new ArrayList<>(text.length());

        // Итерация массива и получение + запись ключей со смещением по шифру
        for (char element : charArray){
            if(((CEASAR_TO_TEXT.get(element))+code) > CEASAR_TO_TEXT.size()){
                intArray.add(((CEASAR_TO_TEXT.get(element))+code) - (CEASAR_TO_TEXT.size()));
            } else intArray.add((CEASAR_TO_TEXT.get(element))+code);
        }

        // Вывод на экран зашифрованного сообщения
        System.out.println("Ваше сообщение в зашифрованном виде: ");


        for (int element : intArray){
            System.out.print(TEXT_TO_CEASAR.get(element));
        }
